...
def inicializar(params: Parametros) -> Sistema:
    ...
    formato_impresion(sistema, "Inicio simulación")
...
def arribo_pedido(sistema: Sistema):
    ...
    formato_impresion(sistema, "Llegada %d unidades" % sistema.cantidad_ordenada)
...
def demanda(sistema: Sistema, params: Parametros):
    ...
    formato_impresion(sistema, "Demanda %d unidades" % tamaño_demanda)
...
def evaluacion(sistema: Sistema, params: Parametros):
    ...
    if sistema.nivel_inventario < params.limite_inferior_inventario:
        ...
        sistema.costo_total_ordenar += params.costo_pedido + params.costo_pedido_por_unidad * sistema.cantidad_ordenada
        ...
        sistema.tiempo_proximo_evento[EventType.ARRIBO_PEDIDO.value] = sistema.tiempo + rng.uniform(
            params.retraso_minimo_pedido, params.retraso_maximo_pedido)
...
def main():
    params = Parametros(
        ...
        costo_pedido=32,
        costo_pedido_por_unidad=3,
        costo_mantenimiento=2,
        costo_faltante=20,
        retraso_minimo_pedido=15,
        retraso_maximo_pedido=30
    )
